...
class ApiController extends ControllerBase
{
    ...
    public function actionSearchDownloads(Request $request)
    {
        ...
        try {
            ...
            // Old Code Commented
            // return new JsonResponse($this->getElasticService()->filter($params), 200);
            $results = $this->getElasticService()->filter($params);
            if (isset($params['queryString']) && !empty($params['queryString']) && isset($results['aggregations']) && !empty($results['aggregations'])) {
                $updatedFilterCounts = $this->getElasticService()->mustFilter($params);
                $results['aggregations'] = $updatedFilterCounts['aggregations'];
            }
