...
namespace Drupal\crescenttool_core\Commands;
 ...
 */
class CtCliService extends DrushCommands {
  ...
  protected function updateHighestExportedSid($submissions) {
    ...
    $this->getCtCoreConfig()->set('warranty_highest_exported_sid', $highest_sid);
    $this->getCtCoreConfig()->save();
  }

  /**
   * Gets the config object for the current module.
   */
  protected function getCtCoreConfig() {
    if (empty($this->ctCoreConfig)) {
      $this->ctCoreConfig = \Drupal::configFactory()->getEditable('crescenttool_core.settings');
    }

    return $this->ctCoreConfig;
  }
  ...
  protected function getHighestExportedSid() {
    return (int) $this->getCtCoreConfig()->get('warranty_highest_exported_sid');
  }
  ...
  protected function uploadToWarrantyFtp($xml_contents) {
    if (!empty($xml_contents)) {
      ...
      // Load config values.
      $config = \Drupal::config('crescenttool_core.settings');
