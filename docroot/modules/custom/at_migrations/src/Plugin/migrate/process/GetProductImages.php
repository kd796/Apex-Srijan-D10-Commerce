...
use Drupal\at_migrations\ImageNotFoundOnFtpException;
use Drupal\at_migrations\ImageOperations;
...
/**
 * Provides a at_get_product_images plugin.
 *
 * Usage:
 *
 * @code
...
class GetProductImages extends ProcessPluginBase implements ContainerFactoryPluginInterface {
   ...
   *
   * @var \Drupal\at_migrations\ImageOperations
   */
  ...
  public function manageAssetMedia(Row $row, $extension = '', $original_extension = '', $user_type_id = '') {
    ...
    $map_media_migration_id = $this->configuration['map_media_migration_id'] ?? 'at_product_media';
    ...
    if ($process_pdf && $get_type == "fid" && $media_type == "pdf") {
      ...
      // Process extension for specific UserTypeID assets.
      $extension = $this->getExtensionforSpecificUserTypeId($user_type_id, $extension);
  ...
  /**
   * Get alternate extension for specific UserTypeId Asset.
   *
   * @param string $user_type_id
   *   UserTypeId for the specific asset.
   * @param string $extension
   ...
   *   Return alternate extension of the asset.
   */
  public function getExtensionforSpecificUserTypeId($user_type_id, $extension) {
    $altered_extension = $extension;
    $extension = strtolower($extension);
    $user_type_id = strtolower($user_type_id);
    $user_type_id_list = [
      'stp file' => 'stp',
    ...
    $user_type_id_file_extension_list = [
      'stp file' => 'ifc',
    ];

    // Alter asset extension only for presence of specific extension.
    if (isset($user_type_id_list[$user_type_id])) {
      if (isset($user_type_id_file_extension_list[$user_type_id])
        &&  $user_type_id_file_extension_list[$user_type_id] == $extension) {
        $altered_extension = $user_type_id_list[$user_type_id];
      }
    }
    return $altered_extension;
  }
