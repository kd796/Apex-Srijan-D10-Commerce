...
/**
 * Provides a ecom_get_product_images plugin.
 *
 * Usage:
 *
 * @code
    ...
    if (!empty($value)) {
      ...
      if (!empty($product)
        ...
        if (!empty($parentProductAssets)) {
          // If we didn't find anything at the product level,
          // then we scan at the parent level.
      ...
      if (empty($final_asset_list) && !empty($this->mediaIds) && empty($this->videos)) {
        if (!empty($this->primaryImageMediaId)) {
          // Combines the media ID of the primary image
          // with the rest of the media IDs,
          // while placing at the beginning of the array.
