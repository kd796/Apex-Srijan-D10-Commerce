...
namespace Drupal\sata_core\Commands;
 ...
 */
class SataCliService extends DrushCommands {
  ...
  protected function getGwCoreConfig() {
    if (empty($this->gwCoreConfig)) {
      $this->gwCoreConfig = \Drupal::configFactory()->getEditable('sata_core.settings');
    }
  ...
  protected function uploadToWarrantyFtp($xml_contents) {
    if (!empty($xml_contents)) {
      ...
      // Load config values.
      $config = \Drupal::config('sata_core.settings');
