(function ($, Drupal) {
  ...
  Drupal.behaviors.torqueConversionCalculator = {
    attach: function (context, settings) {
      $(once('torque-conversion-calculator', '#block-torque-conversion-calculator-button-convert', context)).each(function () {
        $(this).on('click', function() {
          ...
          var conversionType = $('#conversipnType').val();
          var conversionarray = [];
          ...
          jQuery.each(conversionarray, function (index, value) {
            ...
            $('#' + index + '_input').val(conversion);
          });
        });
      });

      $('#block-torque-conversion-calculator-button-reset').on('click', function () {
        $('.block-torque-conversion-calculator input[type="text"]').each(function () {
          $(this).val('0');
        });
      });
    }
