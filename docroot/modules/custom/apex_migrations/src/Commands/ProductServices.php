...
class ProductServices extends DrushCommands {
  ...
  protected function runProductImport(?string $migration_name = '') {
    ...
    if ($result == MigrationInterface::RESULT_COMPLETED) {
      \Drupal::logger('Successfully ran the product migration.', LogLevel::SUCCESS);

      // Success!
      return 0;
    }

    \Drupal::logger('Migration failed with code: ' . $result, LogLevel::ERROR);
   ...
   *   Should we ignore if the previously downloaded filename matches the current?
   *
   * @return int
   ...
   */
  protected function downloadProducts(?string $full_or_delta = '', ?int $search_limit = 10, bool $ignore_same_file = FALSE) {
    ...
    try {
       ...
       * and imported.
       *
       * We could also specify which file to download potentially as a command option.
       */

      $file_result = $this->findFileToUse($all_files, $search_limit, $ignore_same_file);
   ...
   */
  protected function findFileToUse(DirectoryListing $all_files, ?int $search_limit = 10, bool $ignore_same_file = FALSE): ?FileAttributes {
