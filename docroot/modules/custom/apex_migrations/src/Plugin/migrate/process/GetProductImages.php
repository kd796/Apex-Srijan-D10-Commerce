    ...
    if (!empty($value)) {
      ...
      if (!empty($product)
        ...
        if (!empty($parentProductAssets)) {
          // If we didn't find anything at the product level then we
          // scan at the parent level.
      ...
      if (!empty($assetId)) {
        ...
        if (!empty($media_id)) {
          if ($attributeType === 'Primary Image') {
            // Edge case when the main Product and the Group Product
            // Have the Primary Image, then consider the main product
            // Image as the Primary One. mainProductImageId will be
            // pushed to the first item to the mediaIds array.
            if ($level == 'Product Level') {
              $this->mainProductImageId = $media_id;
            }
          }
          else {
            $this->mediaIds[] = [
    ...
    if (!empty($element)) {
      foreach ($element as $single_value) {
        ...
        if ($single_value->getName() === 'MultiValue' && $single_value_attribute_id == 'ExternalVideoURL') {
          ...
          // We only visited the "Values" attribute group so that we
          // could get the video. So leave. NOW!
          break;
        }
      }
    }
    ...
    foreach ($element as $item) {
      ...
      if (!empty($assetId)) {
        if ($attributeType === 'Primary Image') {
          ...
          // If we find the file then we need to reference it in the
          // return array.
          if (!empty($media_id)) {
